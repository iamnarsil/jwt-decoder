...
> [!TIP]
> 👉 點擊驗證結果「**簽章驗證失敗** :x:」訊息連結，將會以 **氣泡訊息** 💬 形式呈現詳細錯誤資訊。
...
> [!TIP]
> 👉 點擊詳細資料表格內說明欄的「**encoded list**」連結時，將會自動複製 **encoded list** 字串至剪貼簿。

<br/>

## 📜 Decoded Disclosures 顯示區
- 分行顯示 SD-JWT Disclosure 基本資訊，並且能夠自動針對 Disclosure 進行**語法高亮顯示** 🎨。

> [!NOTE]
> Disclosure 基礎格式為 **`["salt","name","value"]`**。

<br/>

## 📜 Encoded List 顯示區
- 針對 VC type 欄位為 **StatusList2021** 者，將自動解析 **encodedList** 欄位資料，並且以 **hex dump** 形式呈現。
- 「**手動輸入** 🟦」按鈕：供使用者手動輸入 **encodedList** 資料，以進行狀態清冊驗證。

> [!NOTE]
> 「**手動輸入** 🟦」按鈕只有當 VC type **不等於 StatusList2021** :x: 時才會呈現在畫面。
